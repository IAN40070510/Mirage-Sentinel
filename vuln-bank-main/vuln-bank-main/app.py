        ...
        try:
            ...
            if user and len(user) > 0:
                ...
                if len(user) > 9 and user[9]:
                    return (
                        jsonify({"status": "error", "message": suspension_message}),
                        403,
                    )
...
def dashboard(current_user):
    ...
    user_data = {
        ...
        "is_admin": user[5],
        "profile_picture": (
            user[6] if len(user) > 6 and user[6] else "user.png"
        ),  # Default image
    }
...
def create_virtual_card(current_user):
    try:
        ...
        return (
            jsonify({"status": "error", "message": "Failed to create virtual card"}),
            500,
        )
...
def fund_virtual_card(current_user, card_id):
    try:
        ...
        if not user_result or not card_result:
            return (
                jsonify({"status": "error", "message": "Card or account not found"}),
                404,
            )
        ...
        if usd_amount > float(user_balance):
            return (
                jsonify({"status": "error", "message": "Insufficient main balance"}),
                400,
            )
    ...
    try:
        ...
        if payment_method == "virtual_card" and card_id:
            ...
            if amount > payer_balance:
                return (
                    jsonify({"status": "error", "message": "Insufficient balance"}),
                    400,
                )
