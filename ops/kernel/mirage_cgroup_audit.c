...
#define MIRAGE_PROC_NAME "mirage_cgroup_audit"
...
static struct proc_dir_entry *mirage_proc_entry;

static void mirage_seq_timestamp(struct seq_file *m)
{
...
static int mirage_proc_show(struct seq_file *m, void *v)
{
	...
	seq_puts(m, "mirage_sentinel_kernel_audit=enabled\n");
	mirage_seq_timestamp(m);
...
static int mirage_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, mirage_proc_show, NULL);
}

static const struct proc_ops mirage_proc_ops = {
	.proc_open = mirage_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};

static int __init mirage_cgroup_audit_init(void)
{
	mirage_proc_entry = proc_create(MIRAGE_PROC_NAME, 0444, NULL,
				       &mirage_proc_ops);
	if (!mirage_proc_entry)
		return -ENOMEM;

	pr_info("Mirage-Sentinel cgroup audit module loaded: cgroup=%s memory=%uMiB pids=%u cpu=%u%%\n",
		sandbox_cgroup, memory_limit_mb, pids_limit, cpu_percent);
	return 0;
}

static void __exit mirage_cgroup_audit_exit(void)
{
	proc_remove(mirage_proc_entry);
	pr_info("Mirage-Sentinel cgroup audit module unloaded\n");
}

module_init(mirage_cgroup_audit_init);
module_exit(mirage_cgroup_audit_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Mirage-Sentinel");
MODULE_DESCRIPTION("Kernel-space procfs audit module for Mirage-Sentinel cgroup v2 sandbox policy");
MODULE_VERSION("1.0");
