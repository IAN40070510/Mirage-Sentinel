...
## MSDS (MirSen Defense System) 雙層 AI 防禦架構分工

本專案建議以「輕量級 AI (XGBoost) + 深度 NLP (DistilBERT)」雙層防禦，並依下列三大模組分工協作：

### 組員 A：API Gateway 核心與流量調度
- FastAPI Middleware 攔截所有請求，預處理（URL/Base64 解碼）
- 雙層分流邏輯：數值型參數交給 XGBoost，字串型參數交給 DistilBERT
- 根據 XGBoost 信心水準決定 Pass/Block/Suspect，Suspect 交給 DistilBERT
- 高度可疑流量自動導向 Docker 沙箱（Honeypot Isolation）

### 組員 B：輕量級 AI (XGBoost) 與特徵防線
- 特徵萃取：json_key_count、transaction_amount、頻率特徵等
- 模型訓練與微調：針對 BOLA、BOPLA、GraphQL 越權等資料
- 調參建議：tree_method=hist、scale_pos_weight 處理不平衡
- 效能監控：推論時間需毫秒等級

### 組員 C：深層語意分析與威脅建模 (DistilBERT)
- 微調語料：SQLi、XSS、SSRF、Path Traversal、AI Prompt Injection
- 封裝推論介面供 Gateway 呼叫
- 誤報分析：持續優化模型，避免正常請求被誤擋

#### 團隊合作建議
- 前期由 A 先用 if/else 假資料串 Gateway，B/C 在 Notebook 訓練模型
- 模型穩定後再整合進 Gateway，進行整合測試
- 建議 NLP/DistilBERT 先做 POC 驗證語料與誤報率
