    ...
    try:
        ...
        with httpx.Client(timeout=30.0) as client:
            ...
            if "name" in fake_data:
                ...
                clean_name = "".join(
                    [char for char in tw_name if "\u4e00" <= char <= "\u9fff"]
                )
                fake_data["name"] = clean_name
            else:
        ...
        fallback_data = {
            ...
            "status": "Normal",
            "account_created": (
                now - timedelta(days=random.randint(100, 1000))
            ).strftime("%Y-%m-%d"),
            "last_login": (now - timedelta(days=random.randint(0, 10))).strftime(
                "%Y-%m-%d"
            ),
        }
