...
class SecurityExtractor(BaseEstimator, TransformerMixin):
    ...
    _RE_RCE         = re.compile(
        ...
        r"create_function|preg_replace)\s*\(", re.IGNORECASE
    )
    ...
    def transform(self, X):
        ...
        features_df = pd.DataFrame({
            ...
            'ssti_flag': X_str.str.contains(self._RE_TEMPLATE, regex=True).astype(int),
            'rce_flag': X_str.str.contains(self._RE_RCE, regex=True).astype(int),
            'obf_flag': X_str.str.contains(self._RE_OBFUSCATION, regex=True).astype(int),
            'non_alnum': (X_str.str.len() - X_str.str.count(self._RE_ALNUM)) / L_series,
...
class SentinelModule:
    ...
    def predict(self, df_input) -> pd.DataFrame:
        ...
        decision = np.where(
            attack_score > BLOCK_THRESHOLD,   " BLOCK",
            np.where(
            attack_score >= REVIEW_THRESHOLD, " CALL BERT", " PASS")
        )
