...
def setup_mirage_database():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS deception_logs (
            ...
            raw_payload TEXT,          -- 駭客輸入的原始字串 (How)
            attack_vector TEXT,        -- 系統判定的攻擊類別 (How)
            user_agent TEXT,           -- 駭客使用的瀏覽器或工具指紋 (Who)
    ...
    conn.commit()
    conn.close()
    print(f"[*] Database calibrated at: {DB_PATH}")
