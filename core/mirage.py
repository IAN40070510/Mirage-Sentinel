...
def generate_fake_data(query_id: str, language: str = "zh_TW", client_ip: str = "127.0.0.1", attack_vector: str = "unknown") -> dict:
    """
    使用 AI 欺敵系統生成虛假用戶資料
    ...
    if existing_mem and existing_mem.get("payload"):
        logger.info(f"[欺敵記憶命中] IP:{client_ip} 曾存取過 ID:{query_id}，回傳舊資料。")
